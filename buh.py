...
for number in range(first_number, last_number + 1):
    current_file_name = f"files/{number}.txt"  # Создаем файлы
    with open(current_file_name, 'w', encoding="utf-8") as file:  # Записываем файлы
        file.write(f"Содержимое файла {current_file_name}")
    print(f"Файл {current_file_name} создан")
